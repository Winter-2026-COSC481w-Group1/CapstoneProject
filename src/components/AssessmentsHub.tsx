  ...
  return (
    <div className="min-h-screen bg-stone-50 pt-28 pb-12 px-4 sm:px-6 lg:px-8 text-gray-900 dark:bg-slate-950 dark:text-slate-100 transition-colors duration-300">
      <div className="max-w-6xl mx-auto">
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-gray-900 mb-2 dark:text-slate-100">Assessments</h1>
          <p className="text-gray-600 dark:text-slate-300">View and manage your exams</p>
        </div>

        <div className="flex items-center gap-4 mb-8">
          <button className={"px-4 py-2 " + ((assessmentsFilter === null) ? "bg-emerald-600 text-white" : "text-gray-600 hover:bg-gray-300 bg-gray-200 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700") + " rounded-full font-medium"}
            onClick={() => setAssessmentsFilter(null)}
          >
            All ({assessmentCounts.all})
          </button>
          <button className={"px-4 py-2 rounded-full font-medium " + ((assessmentsFilter === 'pending') ? "bg-emerald-600 text-white" : "text-gray-600 hover:bg-gray-300 bg-gray-200 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700")}
            onClick={() => setAssessmentsFilter('pending')}
          >
            Pending ({assessmentCounts.pending})
          </button>
          <button className={"px-4 py-2 rounded-full font-medium " + ((assessmentsFilter === 'incomplete') ? "bg-emerald-600 text-white" : "text-gray-600 hover:bg-gray-300 bg-gray-200 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700")}
            onClick={() => setAssessmentsFilter('incomplete')}
          >
            Incomplete ({assessmentCounts.incomplete})
          </button>
          <button className={"px-4 py-2 rounded-full font-medium " + ((assessmentsFilter === 'completed') ? "bg-emerald-600 text-white" : "text-gray-600 hover:bg-gray-300 bg-gray-200 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700")}
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredAssessments.map((assessment) => {
            ...
            <div
              key={assessment.id}
              className="bg-white rounded-3xl p-6 shadow-lg border border-gray-200 hover:shadow-xl transition-all group flex-col flex gap-y-2 relative dark:bg-slate-900 dark:border-slate-700 dark:shadow-black/20"
            >
              
              <div>
              <h3 className="text-lg font-bold text-gray-900 mb-1 line-clamp-2 dark:text-slate-100">
                {assessment.title}
              </h3>
              {assessment.status === "completed" && (
                <div className="inline-flex items-center gap-1 px-2 py-1 bg-gray-100 rounded-full text-xs font-medium text-gray-700 mb-3 dark:bg-slate-800 dark:text-slate-300">
              ...
              <div className="absolute top-4 right-4">
                <button
                  ...
                  className="px-2 py-2 hover:text-red-700 rounded-md text-sm font-medium text-gray-700 disabled:opacity-60 disabled:cursor-not-allowed dark:text-slate-300 dark:hover:text-red-400"
                >
              ...
              <div className="space-y-2">
                ...
                <div>
                  <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${
                    assessment.difficulty === 'easy' ? 'bg-green-100 text-green-700 dark:bg-green-500/10 dark:text-green-300' :
                    assessment.difficulty === 'medium' ? 'bg-amber-100 text-amber-700 dark:bg-amber-500/10 dark:text-amber-300' :
              ...
              <div className="flex gap-2">
                <button
                  ...
                  disabled={!!loadingAction}
                  className={"flex-1 flex items-center justify-center gap-2 " + ((assessment.status === "ready" || assessment.status === "completed") ? "bg-emerald-600 hover:bg-emerald-700 text-white dark:bg-emerald-500 dark:hover:bg-emerald-400" : "bg-gray-200 text-gray-600 cursor-default dark:bg-slate-800 dark:text-slate-400") + " py-3 rounded-xl font-semibold transition-colors " + (loadingAction?.startsWith(`start-${assessment.id}`) ? "opacity-75 cursor-not-allowed" : "")}
                >
          ...
          {assessments.length === 0 && (
            <div className="col-span-full text-center py-12">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-3 rounded-xl font-semibold transition-colors dark:bg-emerald-500 dark:hover:bg-emerald-400"
              >
                <Play className="w-4 h-4" />
                Create Assessment
              </button>
